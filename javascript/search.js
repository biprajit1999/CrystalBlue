...
if(keyword!=null && keyword.length>=1) {
    ...
    document.querySelector('.search-result').innerHTML+='<table ><thead><tr><th>Jewellery</th><th>Title</th><th>Price</th><th>Rating</th><th>Purchase</th></tr></thead><tbody></tbody></table>';
    for (let index = 0; index < movieList.length; index++) {
        const element = movieList[index];
        if(element.title.toLowerCase().includes(keyword.toLowerCase())) {
            document.querySelector('.search-result table tbody').innerHTML+='<tr><td><img src="'+element.image+'" alt="image"></td><td>'+element.title+'</td><td>'+element.price+'</td><td>'+element.rating+'</td><td><a href="mdetail.html?id='+element.id+'">Buy Now</a></td></tr>';
        }
    }
}
function search() {
